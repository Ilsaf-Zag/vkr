...
function mapInspection(inspection: any) {
  return {
    ...
    'Автомобиль': inspection.vehicle?.plate_number ?? '—',
...
async function load(query = '') {
  loading.value = true;
  try {
    const { data } = await apiClient.get('/admin/technical-inspections', {
      params: {
        q: query || undefined,
...
async function approve(row: any) {
  ...
  await apiClient.post(`/admin/technical-inspections/${inspection.id}/approve`);
  await load();
}
...
async function submitReject() {
  rejectErrors.value = {};
  try {
    await apiClient.post(`/admin/technical-inspections/${selectedInspection.value.id}/reject`, rejectForm);
...
<template>
  <AppShell>
    <Modal :is-open="showView" title="Техосмотр" @close="showView = false">
      <div v-if="selectedInspection" class="view-content">
        ...
        <div class="view-row"><span class="view-label">Автомобиль</span><span>{{ selectedInspection.vehicle?.plate_number }}</span></div>
        ...
        <div class="view-row"><span class="view-label">Механик</span><span>{{ selectedInspection.mechanic?.full_name || '—' }}</span></div>
    ...
    <Modal :is-open="showReject" title="Отклонить техосмотр" @close="showReject = false">
    ...
    <RegistryPage
      title="Техосмотры"
      description="Заявки на предрейсовые и послерейсовые технические осмотры."
      :columns="['Тип', 'Автомобиль', 'Водитель', 'Путевой лист', 'Запрос', 'Статус']"
      :rows="rows"
      :loading="loading"
      @search="load"
      @select="openView"
